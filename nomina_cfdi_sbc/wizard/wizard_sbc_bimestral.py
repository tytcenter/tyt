...
class CalculoSBC(models.TransientModel):
    ...
    total_empleados = fields.Boolean(string='Incluir empleados sin variables', default=False)
    ...
    def print_sbc_report(self):
        ...
        if self.registro_patronal_id:
            employee = self.env['hr.employee'].search([('registro_patronal_id', '=', self.registro_patronal_id.id)])
            domain.append(('employee_id', 'in', employee.ids))
        ...
        for line in payslip_lines:
            if line.slip_id.employee_id.active == True:
...
#            dias_lab = 0
        ...
        if self.total_empleados:
           for empleado2 in new_employees:
               ...
               worksheet.write(row, 3, empleado2.registro_patronal_id.registro_patronal)
        ...
        if self.total_empleados:
           for empleado2 in new_employees:
               ...
               incidencia = self.env['incidencias.nomina'].create({'tipo_de_incidencia':'Cambio salario', 
                                                                   ...
                                                                   'sueldo_mensual': empleado2.contract_ids[0].wage,
                                                                   'sueldo_diario': empleado2.contract_ids[0].sueldo_diario,
                                                                   'sueldo_diario_integrado': empleado2.contract_ids[0].sueldo_diario_integrado,
                                                                   'sueldo_por_horas' : empleado2.contract_ids[0].sueldo_hora,
                                                                   'sueldo_cotizacion_base': nsbc,
                                                                   'fecha': self.fecha_aplicacion,
                                                                   'contract_id': empleado2.contract_ids[0].id
                                                                   })
